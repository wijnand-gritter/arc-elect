 ...
 * ```
 */
export function SchemaDetailModal({ isOpen, onClose, onEdit }: SchemaDetailModalProps): React.JSX.Element {
    const { modalStack, currentModalIndex, currentProject, navigateToSchema, goBack, setActiveModalTab } = useAppStore();
    ...
    const handleReferenceClick = (reference: SchemaReference) => {
        logger.info('SchemaDetailModal: Handling reference click', {
            reference,
            availableSchemas: schemas.map(s => ({ name: s.name, relativePath: s.relativePath, id: s.id }))
        });
        ...
        if (!referencedSchema) {
            ...
            referencedSchema = schemas.find(schema => {
                const basename = schema.relativePath.split('/').pop()?.replace(/\.(json|schema)$/, '');
                return basename === reference.schemaName || basename === reference.schemaName.replace(/\.(json|schema)$/, '');
            });
        }
        ...
        logger.info('SchemaDetailModal: Reference click result', {
            ...
            foundSchema: referencedSchema ? { name: referencedSchema.name, id: referencedSchema.id } : null
        });
    ...
    logger.info('SchemaDetailModal: Reference data', {
        ...
        referencedByMatches: schema.referencedBy?.map(refId => ({
            refId,
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-7xl max-h-[95vh] overflow-hidden">
                ...
                <div className="flex-1 overflow-hidden">
                    <Tabs value={activeTab} onValueChange={(value) => setActiveModalTab(value as SchemaDetailModalType['activeTab'])}>
                                                    ...
                                                    {schema.content.properties && (
                                                        <div>
                                                            ...
                                                            <div className="space-y-2">
                                                                {Object.entries(schema.content.properties as Record<string, unknown>).map(([key, value]) => {
                                                                    ...
                                                                    const displayType = isEnum ? `${prop.type || 'string'} enum` : propertyType;
                                                                ...
                                                                {references.map((ref, index) => (
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <span className="text-sm">
                                                                            {schemas.find(s => s.name === ref.schemaName)?.metadata.title || ref.schemaName}
                                                                        </span>
                                                                    </div>
                                                                ))}
                                                            </div>
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="space-y-1">
                                                                {referencedBy.map((schema) => (
                                                                    ...
                                                                    >
                                                                        <FileText className="w-4 h-4 text-muted-foreground" />
                                                                        <span className="text-sm">{schema.metadata.title || schema.name}</span>
                                                                    </div>
                                                                ))}
                                                            </div>
                                                        </div>
                                    ...
                                    <Card className="h-full">
                                        ...
                                        <CardContent>
                                            <div className="space-y-4">
                                                ...
                                                {schema.validationStatus === 'invalid' && schema.validationErrors && schema.validationErrors.length > 0 && (
                                                        ...
                                                        {schema.validationErrors.map((error, index) => (
                                                            <div key={index} className="p-3 bg-destructive/10 border border-destructive/20 rounded">
                                                                <p className="text-sm text-destructive font-medium">
                                                                    {error.instancePath ? `Path: ${error.instancePath}` : 'Schema Error'}
                                                                </p>
                                                ...
                                                {schema.validationStatus === 'invalid' && (!schema.validationErrors || schema.validationErrors.length === 0) && (
                                                    <div className="mt-4 p-3 bg-destructive/10 border border-destructive/20 rounded">
                                                        <p className="text-sm text-destructive">
                                                            This schema contains validation errors. Check the schema content for issues.
                                                        </p>
                                                    </div>
                                                )}
                                            </div>
